...
import (
	...
	"github.com/spiffe/go-spiffe/v2/internal/witutil"
...
func FromWITAuthorities(trustDomain spiffeid.TrustDomain, witAuthorities map[string]crypto.PublicKey) *Bundle {
	return &Bundle{
		trustDomain:    trustDomain,
		witAuthorities: witutil.CopyWITAuthorities(witAuthorities),
	}
}
...
func Load(trustDomain spiffeid.TrustDomain, path string) (*Bundle, error) {
	...
	if err != nil {
		return nil, wrapWitbundleErr(fmt.Errorf("unable to read WIT bundle: %w", err))
	}
...
func Read(trustDomain spiffeid.TrustDomain, r io.Reader) (*Bundle, error) {
	b, err := io.ReadAll(r)
	if err != nil {
		return nil, wrapWitbundleErr(fmt.Errorf("unable to read: %v", err))
	}
...
func Parse(trustDomain spiffeid.TrustDomain, bundleBytes []byte) (*Bundle, error) {
	...
	if err := json.Unmarshal(bundleBytes, jwks); err != nil {
		return nil, wrapWitbundleErr(fmt.Errorf("unable to parse JWKS: %v", err))
	}

	bundle := New(trustDomain)
	for i, key := range jwks.Keys {
		if err := bundle.AddWITAuthority(key.KeyID, key.Key); err != nil {
			return nil, wrapWitbundleErr(fmt.Errorf("error adding authority %d of JWKS: %v", i, errors.Unwrap(err)))
		}
	}
...
func (b *Bundle) WITAuthorities() map[string]crypto.PublicKey {
	...
	return witutil.CopyWITAuthorities(b.witAuthorities)
}

// FindWITAuthority finds the WIT authority with the given key ID from the bundle. If the authority
...
func (b *Bundle) AddWITAuthority(keyID string, witAuthority crypto.PublicKey) error {
	if keyID == "" {
		return wrapWitbundleErr(errors.New("keyID cannot be empty"))
	}
...
// Marshal marshals the WIT bundle into a standard RFC 7517 JWKS document. The
...
func (b *Bundle) Equal(other *Bundle) bool {
	...
	return b.trustDomain == other.trustDomain &&
		witutil.WITAuthoritiesEqual(b.witAuthorities, other.witAuthorities)
}
...
func (b *Bundle) GetWITBundleForTrustDomain(trustDomain spiffeid.TrustDomain) (*Bundle, error) {
	...
	if b.trustDomain != trustDomain {
		return nil, wrapWitbundleErr(fmt.Errorf("no WIT bundle for trust domain %q", trustDomain))
	}

	return b, nil
}

func wrapWitbundleErr(err error) error {
