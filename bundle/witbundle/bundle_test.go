...
func TestFromWITAuthorities(t *testing.T) {
	...
	b := witbundle.FromWITAuthorities(td, witAuthorities)
	require.NotNil(t, b)
	assert.Equal(t, b.WITAuthorities(), witAuthorities)
}
...
func TestLoad(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			err: "witbundle: unable to read WIT bundle: open testdata/does-not-exist.json: " + errstrings.FileNotFound,
		},
		{
			tf:  testFiles["missing kid"],
			err: "witbundle: error adding authority 1 of JWKS: keyID cannot be empty",
		},
	}

	for _, testCase := range testCases {
		t.Run(testCase.tf.filePath, func(t *testing.T) {
			bundle, err := witbundle.Load(td, testCase.tf.filePath)
			if testCase.err != "" {
			...
			assert.Len(t, bundle.WITAuthorities(), testCase.tf.keysCount)
		})
	}
}
...
func TestRead(t *testing.T) {
	testCases := []struct {
		...
		{
			tf:  testFiles["non existent file"],
			err: "witbundle: unable to read: invalid argument",
		},
		{
			tf:  testFiles["missing kid"],
			err: "witbundle: error adding authority 1 of JWKS: keyID cannot be empty",
	...
	for _, testCase := range testCases {
		t.Run(testCase.tf.filePath, func(t *testing.T) {
			...
			bundle, err := witbundle.Read(td, file)
			if testCase.err != "" {
				require.EqualError(t, err, testCase.err)
				return
			}
...
func TestParse(t *testing.T) {
	testCases := []struct {
		...
		{
			tf:  testFiles["non existent file"],
			err: "witbundle: unable to parse JWKS: unexpected end of JSON input",
		},
		{
			tf:  testFiles["missing kid"],
			err: "witbundle: error adding authority 1 of JWKS: keyID cannot be empty",
	...
	for _, testCase := range testCases {
		t.Run(testCase.tf.filePath, func(t *testing.T) {
			...
			bundle, err := witbundle.Parse(td, bundleBytes)
			if testCase.err != "" {
				require.EqualError(t, err, testCase.err)
				return
			}
...
func TestWITAuthoritiesCRUD(t *testing.T) {
	// Test AddWITAuthority (missing authority)
	b := witbundle.New(td)
	err := b.AddWITAuthority("", "test-1")
	require.EqualError(t, err, "witbundle: keyID cannot be empty")
	...
	authority, ok := b.FindWITAuthority("key-1")
	require.True(t, ok)
	require.Equal(t, "test-1", authority)

	authority, ok = b.FindWITAuthority("key-3")
	require.False(t, ok)
	require.Nil(t, authority)

	require.Equal(t, true, b.HasWITAuthority("key-1"))
	b.RemoveWITAuthority("key-3")

	require.Equal(t, 2, len(b.WITAuthorities()))
	require.Equal(t, true, b.HasWITAuthority("key-1"))
	require.Equal(t, true, b.HasWITAuthority("key-2"))
...
func TestMarshal(t *testing.T) {
	...
	bundle, err := witbundle.Load(td, "testdata/jwks_valid_2.json")
	require.NoError(t, err)
	...
	bundleParsed, err := witbundle.Parse(td, bundleBytesMarshal)
	require.NoError(t, err)
...
func TestGetWITBundleForTrustDomain(t *testing.T) {
	b := witbundle.New(td)
	b1, err := b.GetWITBundleForTrustDomain(td)
	require.NoError(t, err)
	require.Equal(t, b, b1)

	td2 := spiffeid.RequireTrustDomainFromString("example-2.org")
	b2, err := b.GetWITBundleForTrustDomain(td2)
	require.Nil(t, b2)
