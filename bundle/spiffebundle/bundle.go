	...
	for i, key := range jwks.Keys {
		...
		case witSVIDUse:
			if err := bundle.AddWITAuthority(key.KeyID, key.Key); err != nil {
				return nil, wrapSpiffebundleErr(fmt.Errorf("error adding WIT authority %d of JWKS: %v", i, errors.Unwrap(err)))
			}
		}
	}
...
func FromWITBundle(witBundle *witbundle.Bundle) *Bundle {
	...
	bundle.witAuthorities = witBundle.JWTAuthorities()
	return bundle
}
...
func (b *Bundle) WITBundle() *witbundle.Bundle {
	...
	return witbundle.FromJWTAuthorities(b.trustDomain, b.witAuthorities)
}
